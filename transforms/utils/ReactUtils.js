// FIXME importing react-codemod package breaks jest tests execution

/**
 * https://github.com/reactjs/react-codemod/blob/master/transforms/utils/ReactUtils.js
 *
...
export default (j) => {
  ...
  const findReactCreateClassCallExpression = path => (
    j(path).find(j.CallExpression, { callee: REACT_CREATE_CLASS_MEMBER_EXPRESSION })
  );
  ...
  // Finds all classes that extend React.Component or React.PureComponent
  const findReactES6ClassDeclaration = (path, classNames) => {
